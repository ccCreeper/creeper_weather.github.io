...
def ProcessData():
    ...
    # 读取测试集和验证集
    X = pd.read_csv("date_train.csv", index_col="日期", parse_dates=True, date_format='%Y-%m-%d')
    y = pd.read_csv("date_valid.csv", index_col="日期", parse_dates=True, date_format='%Y-%m-%d')
